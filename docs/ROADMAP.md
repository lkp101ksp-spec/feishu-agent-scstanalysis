...
| 2026-09-07 | Phase 38 自然语言意图预判（ut-6 产品决策点落地，用户选定「意图预判+自动转研究（确认闸）」）：IntentGateService LLM 轻量分类 → 疑似研究意图发确认卡（确认执行/忽略，owner 内嵌 + 内存幂等 + TTL 1800s 过期失效，对齐 code_approval 模式不落库）→ 确认后经 research_intent 回调转 ResearchRunner（等同 /research）；分类/发卡异常一律回退闲聊（绝不阻断正常聊天）；单挂载点设计（只挂 orch，回调经 ctx.orchestrator 同源取，根上规避 ut-7 双挂载事故）+ 卡片原地换面去按钮；settings INTENT_GATE_ENABLED/TTL；审计 target_id 链补 intent_id + research_intent_approved 专项审计；toolsv1/ 参考仓库按用户决策移出归档 I:\archive\toolsv1；TDD 新增 25 用例，全量回归 **1088 passed**；真机验收待人工 |
