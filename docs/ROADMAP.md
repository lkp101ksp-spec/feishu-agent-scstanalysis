...
| 2026-09-17 | **slingshot 锚定 sanity 结构化落地**（翻案教训收口，用户"按你的建议来"）：bridge 导出 sling_lineages.csv（metadata$lineages 谱系路径 lineage,path,start,end）→ pseudotime.py 读回（缺失严格 RuntimeError=部署漂移）out 增 lineage_starts/lineage_paths/**anchor_ok**/anchor_note 四字段——锚定生效判据从"pst 最小段簇构成"猜测口径升为 R 侧谱系起点权威口径；冒烟⑭a0（starts 唯一=trunk+paths 全 trunk 开头）+⑳同步断言全 21 场景绿；**10k 真机复验**（翻案现场 pt10ksmoke）：anchor_ok=True/starts=10×"16"，旧口径误判库上新口径一击命中；另：本机 git 对象库只读 rename 被过滤层拦截（当日突发，疑系统更新/EDR），core.createObject=link 本仓库绕行钉档 |
| 2026-09-17 | **CytoTRACE2 死刑推翻——R 版离线翻案探针**（用户点名评估）：第三十五段死刑只覆盖 Python 包（torch+GDrive gdown）；官方 R 版 digitalcytometry/cytotrace2 模型参数 parameter_dict_19.rds 等 5 件**随 inst/extdata 分发**——镜像 Depends 大头（Seurat 全家）CellChat 层已带，仅缺 HiClimR/Rfast（apt g++/gfortran/cmake/libnetcdf-dev 构建期+CRAN 10.2min+GH tarball 13.4s/23MB）；离线审计 grep 零运行期下载；合成 1200c 2.1min+**10k 真机 --network none 断网 11.9min 全离线跑通**（14133 特征基因 100% 映射）；生物学口径钉注：HSC marker 第一的簇 16 绝对干性 rank 15/24 稳定复现=跨组织校准口径与 marker 打分语义分歧非缺陷；**终判可立 Phase**（独立 sc_cytotrace2，关键约束=输入须 raw/CPM 计数，生产 processed X 已 scaled 需接 merge_10x 形态上游）；探针资产 bio:c2probe 镜像+_eval 四脚本留档 |
| 2026-09-17 | **Phase 63：sc_cytotrace2 集成落地——CytoTRACE2 绝对干性打分独立工具**（死刑推翻探针终判后用户"先立 Phase"）：bio.Dockerfile 增 CytoTRACE2 层（apt 构建期 5 件+CRAN HiClimR/Rfast+api.github.com tarball 装包+purge+自检，Seurat 全家 CellChat 层存量复用）；cytotrace2_bridge.R（tsv→五列 csv+meta json，映射率 feats[[2]] 口径上报）；cytotrace2.py **整数计数定位链**（filtered→raw→processed counts layer/X/.raw 三路抽查前 200 细胞非负+整数率>0.99，全不命中 INVALID_INPUT 提示 merge_10x 形态上游——log/scaled 硬约束以拒收落地）+基因重名防御+preKNN 口径 by_cluster+五列写回+UMAP；L3 注册 timeout 1800（附录 A 表+SECTION_TITLES 两连带同步由契约测试守住）；冒烟 4 场景 259s 绿（合成库只断言链路结构，基因名取真实特征集）；真机 c2real（merge_10x 条码对齐 1000×32285）133s 绿——mapped 14133、potency 真实梯度 Multi316/Oligo282/Diff232、簇 16 rank 14/22 对照 10k 15/24 语义分歧再钉，负路径 pt1ksmoke 生产形态正确拒收；门禁 ruff/mypy 双平台+pytest 1336 基线维持（3 failed 连带表项已补绿）；推送闭环 97b8791+615d393 双 commit 上去，CI 35247728669 test+docker-smoke 双绿 |
| 2026-09-18 | **Phase 63 后四线收官**（用户"好，1-4按顺序来"）：①q1 CI 直连验证——bio-image-smoke manual job 四 job 全绿 1h0m31s（CytoTRACE2 层 api.github.com tarball 端点 CI 直连构建实证），4fc2069 推送闭环（昨夜 32% 崩溃定性 runner 抖动）；②q2 NicheNet 评估**终判可立 Phase**——装包配方通（apt gdtools 编译链 libcairo2-dev 等 4 件+CRAN 现行组+remotes::install_version archive 组 BBmisc/ParamHelpers/mlr/mlrMBO+本体 tarball，nichenetr 2.2.1.1）、Zenodo 7074291 先验 433MB 断网可用、合成自证 A2M rank 1/68 aupr 0.985（v2 tibble 口径三坑：$test_ligand 列非 rownames/背景须含配体自身/剔全零配体列），资产 bio:nnprobe 镜像+探针脚本留档；③q3 sc_cytotrace2 **59900 全量适配三段落地**——稀疏导出 mmwrite field=integer+R readMM dgCMatrix（内存口径 nnz×12B，工具"59900 级会爆"旧限制解除）、parallelize_models 钉死 FALSE（fork 坑：默认 TRUE 走 predictData 内 mclapply 大矩阵子进程异常输出空 seq wrong sign，FALSE 仅影响模型层 subsample 层仍并行）、timeout 1800→7200；32g OOM 根因=c2 主函数内部强制 sparse→dense coercion 14.4GiB（55g 容器 2h15min 通过）；验证三层：冒烟 264s+c2real 逐字段一致（mapped 14133/rank16 14/22）+59900 全量产物段 finish_c2_59900.py；**三口径 cluster16_preKNN_rank 1k 14/22、10k 15/24、59900 13/24 跨量级稳定**；大库纪律口径修正：subset ~1000 限探索期反复验证，全量单次结论可行但需 ≥55g 容器直跑；④q4 TCR 免谈（全树无 VDJ contigs） |
| 2026-09-18 | **Phase 64：sc_nichenet 集成落地——NicheNet 配体活性优先级独立工具**（八补记 q2 终判后用户"做nichenet"）：bio.Dockerfile 增 NicheNet 层 354s（gdtools 编译链四件+CRAN 现行组+archive 组 mlrMBO+本体 tarball 走 PROXY+**Zenodo 7074291 先验四件烘焙 /opt/nichenet_prior**：human lt 33354×1226/mouse lt 22521，R 依赖存量复用构建远快于探针）；**KB 级统计轻量桥**（矩阵本体不出 Python——sender/receiver 掩码表达比例统计表+geneset.csv 传 R，桥接载荷任意规模恒 KB 级，对比 q3 前矩阵桥接大库免疫）；nichenetr_bridge.R（剔全零配体列+bg 并入配体两探针坑结构化落地+meta 手写 sprintf）+ nichenet.py（X scaled 回退 .raw/species 空=自动探测+style_guard 快败/条图+配体×靶基因热图）；L3 注册 timeout 1200（附录 A+SECTION_TITLES 同步，守护测试 14 passed 42 工具自动入列）；冒烟五场景绿（先验真实基因名导出+**A2M 自证 rank 1/60**+mouse 自动探测+三拒收；插曲：构建快照早于工具写入 COPY 层旧内容秒级重建、np.ndarray.data 是 memoryview 统一 .min()）；真机 pt10ksmoke 绿（receiver=簇 16 HSC 83 细胞，geneset 12/12 映射，**389 配体可测**，HSC niche 轴 **Angpt1 进 top30**，top=Adam17/Fgf10/Tnf/Vegfc 方向合理）；门禁 ruff/mypy 双平台绿（_pct Any 收敛三处） |
| 2026-09-18 | **Phase 64 后四线收官**（用户"依次实现前面说的1-4"）：①CI 直连验证挂载——9b31968 ci.yml bio-image-smoke 加挂 _smoke_sc_nichenet 断网冒烟（sc_cellchat 后追加，步骤 0 从镜像先验导基因名 runner.temp 全新 _nnpeek），manual run 35314955243 触发 test/docker-smoke 双绿先行、st/bio 镜像 job 构建中（bio 层 50-71min）；轮询插曲 REST API repo 名写错 404→gh run list 纠正；②NicheNet×CellChat 交叉（c2real 1000 鼠源，物种疑云解除=detect_symbol_style 自动探测兜住）——95 配体并集 **both 48**/nn_only 35/cc_only 12，双证据头 Lamb1(cc9 LAMININ)/Ptn(cc10 PTN)/Col4a2(cc13 COLLAGEN)/Vegfa(nn10 VEGF)，**rho=-0.278 负相关=概率法与活性法排序互补**，both 交集即高置信 niche 候选；③供体级三路条件检验（伪重复叙事复现）——group 列铁证从 fate_prob_cond_59900.log 列联表恢复（**G1={M2,M6,M8_9,M10,M13_14} 30724/G2={M18,M19,M90,M93_81,M96} 29176** 与 c2real 值域 100% 对齐），c2donor 副本 G1=482/G2=518：sc_de 细胞级 270 显著（top up Arf5/Gnb2/Scand1 与 59900 全量同款）→供体级 **0/16429**、cellfreq 细胞级 3 簇→供体级 **0/22**（q≥0.524）、pseudotime 双引擎不显著（slingshot p=0.421/monocle3 p=0.548）——细胞级虚高→供体级归零完整复现；插曲 odds_ratio None（Fisher 表退化）格式化防御；④st 侧探针双层验证——A 层假名反卷积产物 INVALID_INPUT 拒收（0/20）+B 层真实符号 600 spot 合成库跑通（60 配体，A2M top10/200/200），anndata 0.11+ nullable StringDtype 写盘修复（allow_write_nullable_strings=False 一行），结论=工具零改动可读空间 AnnData、st 扩展=调用方按空间邻域定义 sender-receiver、候选暂不立 Phase；门禁 pytest 1336 passed 基线维持，project_rules.md 大库纪律第三版入档 |
| 2026-09-18 | **Phase 64 后续三线（用户三选全做）**：①CI 挂账根因修复——run 35314955243 验尸：bio 构建 2h32m 成功+cellchat 冒烟过，挂 sc_nichenet 冒烟步骤 0 宿主侧 mkdir EACCES（docker -v 以 root 自动创建 bio_ws，runner 建目录被拒；本机 Windows 属主映射未暴露=平台差异盲区），修复=ci.yml bio/st 两 job 冒烟前 runner 预创建目录+actions v5/v6（Node24）+runs-on 钉 ubuntu-24.04（防 10 月 latest 迁移），3b1bc2d push run 3m16s 绿、manual re-run 35329154163 触发（~2.5h 后台）；②both-48 产物化——c2real/niche_ligands_both48.csv（48 配体按 nn_aupr 降序+cc_prob/通路/**最优 sender 簇**列；Thbs1 cc_rank=2 两法一致强信号，sender 簇分散=niche 多源）；③st_nichenet 立项评估设计探针终判**可立 Phase**——probe_st_nichenet2.py 地面真值（600 spot 网格 core/r1/r2/far，A2M 仅一环+5 诱饵仅远端）三路判据：空间口径 A2M rank1+诱饵 0/5 邻近剔除、非空间诱饵 5/5 tested 48→53、洗牌负对照 5/5≈非空间——候选集差异确由空间邻近驱动；机理钉注：min_expr 门控 sender 侧、活性排序先验驱动空间信息落点=候选集过滤；首跑教训=配体列基线 Poisson 噪声骗过 min_expr 须严格分区清零；单次 13s 分环毫秒级，逐 niche 对循环可行（timeout 1800） |
| 2026-09-18 | **Phase 65：st_nichenet 集成落地——空间版 NicheNet 配体活性优先级（st 第 16 工具）**（立项探针三判据终判后用户"可以，st做起来"；spec 2026-09-18-st-nichenet-design.md）：跨镜像分发复用 st_cellchat_v2 先例——st_nichenet.py 物理在 sc_tools/、l3_spatial 注册 image=bio、st 镜像零增重；空间增量=BFS kNN 分环（cKDTree k+1 近邻+deque 扩散，core=receiver niche 全 spot，r1..max_rings 为 sender，远端 -1），空间约束=候选母体收窄（min_expr 门控在收窄后 sender 母体统计，远端高表达配体 sender_pct=0 被过滤——诱饵地面真值机制）；KB 桥/绘图/emit 全范式继承 sc 版（expr_stats.csv+geneset.csv → nichenetr_bridge.R 零改动），新增 rings.png 身份产物（argsort 远端先画 core 压顶）+ring_counts；参数 receiver_niche 必填/max_rings 1..5 默认 1/knn 4..20 默认 6/timeout 1800（附录 A+SECTION_TITLES 连带同步，表守护测试按设计抓漏后补绿 21 passed）；TDD 5 用例+st 15→16；冒烟六场景绿（20×20 三区网格 R/S/F：human A2M top10 自证+诱饵 0/5 tested+sender 22/400+ring_counts 双断言、mouse 自动探测 87 配体、假 geneset/坏 niche/坏 groupby/无 spatial 四拒收；两坑钉档：冒烟全基因基础 poisson(1.0) 下诱饵 sender_pct≈0.63 骗过 min_expr→非远端区严格置零落实地面真值、本地/CI 镜像不含新脚本→冒烟 BASE 加 sc_tools 整目录挂载对齐 handler script_dir 行为）；门禁 ruff/mypy（CI 口径 196 文件 0 错）/全量回归 **1335 passed**；ci.yml sc_nichenet 后追加 st 冒烟步骤（BIO_WORKSPACE_ROOT env 同口径） |
| 2026-09-18 | **Phase 66：OSCC Visium 真机链验证——st 三工具首遇真实数据全绿，st_nichenet 真机首验**（用户提供 oscc_visium.h5ad 1749 spots×15624 genes、pathologist_anno.x 病理注释六类，兑现 Phase 65"st 真机验证"挂账）：探针 probe_oscc_st.py 容器链 st_process（**8 空间域**）→st_commot（**324 LR**，top COLLAGEN，MIF-CD74_CD44 2856 榜首）→st_cellchat_v2（**556 LR**，top TGFB1_TGFBR1_TGFBR2 SCC→SCC pval=0，EGF 家族 SCC↔正常黏膜）→st_nichenet 双 receiver 分环闭合（SCC core 1055/r1 110/远端 584；Lymphocyte Positive Stroma core 123/r1 99/远端 1527；SCC top CDH1/IGFL2/ADM/SPP1/FURIN，LP top CXCL1/ADM/COL4A5/HLA-DRB1/CXCL8/JAG1/FURIN）→交叉解读：**SCC∩LP 共享 ADM+FURIN 与 cellchat TGFB1 pval=0 三证据互证同一 TGFB/缺氧轴，JAG1/2 呼应 commot NOTCH，SCC 浸润轴 CDH1/SPP1/MMP9/TNFSF10，LP 免疫签名 CXCL1/8+HLA-DRB1，commot 基质轴（CD44 反复出现）与 cellchat EGF 轴互补**；真机实锤并根治**多 receiver 产物覆写真 bug**（LP 覆写 SCC 同名 CSV/PNG——五件产物加 receiver slug 后缀 re.sub[:40]，冒烟断言 `_r` 同步+handler 描述同步）；CI 挂账 35329154163 验尸=run 实为 failure，死因 bio-image-smoke 冒烟步宿主缺 anndata（冒烟脚本宿主侧 import、ci.yml 只装 pandas，存量 bug 非本次引入）→ci.yml 补 anndata，manual run 35340788471 双 image-smoke 全绿（st 11min/bio ~1.5h，sc_nichenet 冒烟步即死点复验通过）挂账闭环；探针交叉段键名两处修正（top_ligands dict/interaction）；门禁 ruff/mypy 196 文件/SMOKE OK/pytest **1341 passed**（=1335+6 pg 口径差非回归） |
| 2026-09-18 | **Phase 67：OSCC 全 niche 逐 receiver NicheNet 编排探针——5 niche 一次全绿 + pg 真库层清账**（用户"继续做什么"，探针先行未立 spec）：编排探针 probe_oscc_allniche.py 复用 probe_oscc_st 五件套（import 零拷贝）——枚举 pathologist_anno.x ≥5 spot 达标 **5/6 niche**（NA 2 spot 落选）→数据驱动 geneset top30→逐个 st_nichenet（SCC 110/LN 64/LP 99/Mucosa 48/Glandular 17 sender）→**43 配体×5 niche aupr_corrected 矩阵+热图+三源交叉 summary 一次落盘**；slug 后缀断言入探针（每轮自动验收，Phase 66 修复真机坐实：4 类产物×5 niche+rings 共 25 文件零覆写；ds 根目录无后缀旧产物=Phase 66 前历史残留钉注）；交叉结论：EGF 轴（HBEGF/AREG）双证据仅正常黏膜/腺体=**正常上皮锚点**、缺氧 ADM 跨 3 niche=全图性、免疫梯度配体对偶（LN CD47 逃逸 vs LP CXCL1/8+HLA-DRB1 激活）、commot 通路层普适（COLLAGEN/LAMININ/MIF 全 niche）vs nichenet 配体层特异=两工具正交再证、三大肿瘤/基质 niche∩cellchat 为空属发现=空间约束改写全图聚合排序（st_nichenet 价值主张）；画像留存零新代码（profile_dataset 直接吃自定义目录名→_profiles/oscc.json：1749×15624/genes 中位 1615/mt 2.11%/upper→human）；**pg 真库层 6 passed 15.25s**（compose postgres 常驻，pre-push deselected 挂账清零，本机 1341 全集闭环） |
| 2026-09-18 | **Phase 68：st_niche_scan 集成落地——全 niche NicheNet 批量扫描工具化（st 第 17 工具）**（Phase 67 探针升格，用户"好，继续"；spec 2026-09-18-st-niche-scan-design.md）：st_niche_scan.py 薄编排层（sc_tools/，枚举 groupby ≥min_spots 全 niche → _niche_up_genes 数据驱动 geneset top30 剔 MT-/RPL/RPS（探针逐位同口径）→ subprocess 自调同目录 st_nichenet.py 零改动黑盒复用 → 墙钟预算自管（单 niche cap 1800s/收尾余量 180s/TOOL_TIMEOUT 3600s，超预算 niche 记 TIME_BUDGET 进 failures 不中断）→ aupr 宽表+热图+summary 三件落盘）；L3 注册 image=bio/script_dir=_SC_SCRIPT_DIR/timeout 3600（handler+ToolSpec+SECTION_TITLES+附录 A 3600 行+ci.yml 冒烟步五连带，契约测试 2 passed 守护）；TDD+清单守护 14 passed（函数名 fourteen 历史遗留只改清单）；**冒烟 5 轮三教训钉档**——①R 区无特意上调基因→geneset 与 A2M 靶零交集（aupr 机理=先验靶在 receiver geneset 富集，非 sender 高表达）②geneset 混噪声稀释区分度（全体 0.64+）→n_geneset=20 与 a2m_hits 等宽 100% 纯自证③**fail() 终止语义三分法**（fail 只 emit 不终止：main 内+return/子函数内+raise SystemExit(1)/run() 重抛——裸 return 双 JSON 事故根治）；**真机 oscc 对拍两轮全绿**（probe_oscc_nichescan_tool.py：首轮 5/5 niche aupr 逐值一致仅缺 ring_counts 字段，补字段+mypy 7 错修复——签名 5→4 元组注解真 bug（运行时无害门禁必抓）/裸 dict ×4/Any ×1——后二轮 5/5 逐值全对齐含 ring_counts，生物学 SCC=CDH1/IGFL2/ADM、LP=CXCL1/ADM/COL4A5 与探针逐位复现零语义漂移）；门禁 ruff/mypy 双平台/pytest **1341 passed** 零回归 |
