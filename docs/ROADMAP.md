...
| 2026-09-07 | /model 池扩充 Kimi（用户提供 coding plan url+key）：llm.yaml providers 加 name=kimi（LLM_PROVIDER_KIMI_* 三 env，Phase 30 预留扩展流程）+ .env 真值 + .env.example 示例更新为 api.kimi.com/coding/v1；模型名 GET /models 实测选定 kimi-for-coding（K2.7 Coding 256k；另有 highspeed/k3/k3-256k 可追加）；连通性 chat/completions 200 出 pong（reasoning_content 独立字段不污染 content，项目极简 payload 原生兼容；Windows curl -d 转义坑，验证走 Python httpx）；config_check OPTIONAL 遵循 Phase 30 决策不登记池键位；全量回归 **1150 passed**；ws_client pid=49064；待真机 /model 卡片验证 |
