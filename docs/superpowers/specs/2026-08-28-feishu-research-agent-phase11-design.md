...
### 14.5 群消息权限开通（2026-08-29 22:07 真机验证 ✅）

遗留项 1 已处理完毕：

- **权限申请**：后台开通 `im:message.group_msg`（获取群组中所有消息，敏感权限）并创建版本发布生效
- **前置代码侧群聊门控**（commit `b49e219`）：开通前预判刷屏风险——`process()` 原对非指令消息一律 LLM 回复，群权限开通后每条闲聊都会刷屏。实施：
  - `IncomingMessage` 增加 `chat_type` 字段，normalizer 从 payload 透传（ws/webhook 双入口）
  - `process()` 在指令路由后插入门控：`chat_type=="group"` 且非指令（非 `/` 开头、非 `#写到`）→ `skipped: group_non_command` 静默忽略；私聊（p2p）行为完全不变
- **真机验证**（2026-08-29 22:07，群内实测）：

| 验证点 | 结果 |
|---|---|
| 群闲聊静默忽略（防刷屏门控） | ✅ `22:07:10` 闲聊 → `skipped: group_non_command`，无回复 |
| 群内指令正常处理 | ✅ `22:07:26` `/template-list` → `template_listed: ['tpl_test']`，群内收到回复 |

### 14.6 全量评论提醒开关（2026-08-29 23:15 真机验证 ✅）

遗留项 3 已处理完毕：

- **commit `e550697`**：`CommentNotifyService` 加 `notify_all` 参数——True 时所有新评论都推 IM（标题 `[评论提醒]`，纯闲聊列表不附 `/comment-apply` 提示，混合时保留）；False 保持原防噪音行为（只推指令评论）。runtime 三处注入，环境变量 `COMMENT_NOTIFY_ALL=1/true` 控制，默认关
- **真机验证**（2026-08-29 23:15）：`.env` 置 `COMMENT_NOTIFY_ALL=1` 重启后，普通评论（无指令）事件 → 1.2s 推送 `notified: 12`（1 新评论 + 11 条历史从未通知的 pending 普通评论——去重日志此前只记指令评论，属预期）。注：验证时先遇 `ignored_unbound`（绑定 TTL 30 分钟已过期），重新 `/bind-doc` 后通过

### 14.4 遗留与后续

1. ~~**群消息权限**~~：✅ 已开通并真机验证通过（见 §14.5），群聊可用（指令照常、闲聊门控防刷屏），私聊不受影响
2. 联调过程再现「双 ws_client 进程抢事件」坑（系统 Python 与 venv 同时起）：已按 §6 排障清理，惯例为清旧再单实例启动
3. ~~全量评论提醒开关~~：✅ `COMMENT_NOTIFY_ALL` 已实现并真机验证通过（见 §14.6）；当前部署已开启
