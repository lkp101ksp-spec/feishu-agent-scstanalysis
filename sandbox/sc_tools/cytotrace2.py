...
主链：计数矩阵 genes×cells tsv 导出 → Rscript
...
~1000（project_rules.md）；本工具 dense 化导出 tsv 内存口径
= cells×genes×8B，59900 级全基因会爆——按纪律用子集。
"""
...
OUT_COLS = ["CytoTRACE2_Score", "CytoTRACE2_Potency",
...
def _is_counts(x: Any, n_sample: int = 200) -> bool:
    ...
    return bool(x.size and np.all(x >= 0)
                and float(np.mean(x == np.round(x))) > 0.99)
...
def main() -> None:
    ...
    df = pd.DataFrame(xd.T, index=genes, columns=cells)
    df.index.name = "gene"
    expr_tsv = in_dir / "expr.tsv"
    df.to_csv(expr_tsv, sep="\t", index=True)
    del df, xd

    r_cmd = ["Rscript", R_BRIDGE, str(expr_tsv), str(in_dir), species,
             str(ncores)]
    if batch_size > 0:
    ...
    if proc.returncode != 0:
        fail("SCRIPT_ERROR",
             f"Rscript cytotrace2_bridge.R failed: {proc.stderr[-1200:]}")
        return
    res_p = in_dir / "c2_result.csv"
    if not res_p.exists():
        fail("SCRIPT_ERROR",
             "cytotrace2_bridge.R did not produce c2_result.csv; "
    ...
    write_p = (ds_dir / "processed.h5ad"
               if (ds_dir / "processed.h5ad").exists() else src)
    aw = ad.read_h5ad(write_p)
    al = res.reindex(pd.Index(aw.obs_names.astype(str)))
    aw.obs["CytoTRACE2_Score"] = al["CytoTRACE2_Score"].to_numpy(
        dtype=float)
    aw.obs["CytoTRACE2_Relative"] = al["CytoTRACE2_Relative"].to_numpy(
        dtype=float)
    aw.obs["preKNN_CytoTRACE2_Score"] = al[
        "preKNN_CytoTRACE2_Score"].to_numpy(dtype=float)
    aw.obs["CytoTRACE2_Potency"] = pd.Categorical(
        al["CytoTRACE2_Potency"].astype(str))
    aw.obs["preKNN_CytoTRACE2_Potency"] = pd.Categorical(
        al["preKNN_CytoTRACE2_Potency"].astype(str))
    ...
    if cluster_col in aw.obs:
        ...
        by = (pd.DataFrame({"cluster": cl,
                            "preKNN": aw.obs[
                                "preKNN_CytoTRACE2_Score"].to_numpy()})
              .groupby("cluster")["preKNN"]
              .agg(["mean", "count"])
              .sort_values("mean", ascending=False))
        by_csv = str(ds_dir / "cytotrace2_by_cluster.csv")
        by.round(4).to_csv(by_csv)
        top5 = {str(k): round(float(v), 4)
                for k, v in by["mean"].head(5).items()}
    ...
    if "X_umap" in aw.obsm:
        ...
        s = ax.scatter(umap[:, 0], umap[:, 1], s=4, c=sc,
    ...
    potency_table = aw.obs["CytoTRACE2_Potency"].astype(str) \
