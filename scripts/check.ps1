...
Write-Host "`n== [1/3] ruff check ==" -ForegroundColor Cyan
& $ruff check .
...
Write-Host "`n== [2/3] ruff format --check (informational) ==" -ForegroundColor Cyan
...
Write-Host "`n== [3/3] pytest 默认层 + cov ==" -ForegroundColor Cyan
& $py -m pytest -q -m "not pg" --cov --cov-report=term
