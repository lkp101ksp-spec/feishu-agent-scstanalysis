...
def client_with_gate():
    """真 IntentGateService + MagicMock research_runner 的卡片环境。"""
    llm = MagicMock()
    ...
    return TestClient(app), gate, runner, factory
...
def _payload(iid, *, decision="approve", owner="ou_1", open_id="ou_1"):
    return {"action": "research_intent", "intent_id": iid,
...
def test_approve_routes_to_research_runner(client_with_gate):
    ...
    client, gate, runner, factory = client_with_gate
    iid = _offer(gate)
...
def test_approve_audited_with_intent_target(client_with_gate):
    """批准落专项审计：research_intent_approved，target_id=intent_id。"""
    client, gate, runner, factory = client_with_gate
    iid = _offer(gate)
    ...
    try:
        rows = [r for r in AuditRepo(s).list_recent(limit=20)
                if r.action == "research_intent_approved"]
    finally:
        s.close()
    assert len(rows) == 1
